...
class TestNovelProblemHandling:
    ...
    def test_common_problem_detection(self):
        ...
        assert result.get('is_novel', False) == False
    ...
    def test_novel_problem_debugging(self):
        ...
        assert 'systematic' in results['reasoning'].lower() or 'novel' in results['reasoning'].lower()
    ...
    def test_physics_reasoning_independence(self):
        ...
        assert 'novel' in results['reasoning'].lower() or 'systematic' in results['reasoning'].lower()
        
        # Should still identify that analysis is needed
        assert any('systematic' in issue.solution.lower() for issue in results['issues'])
