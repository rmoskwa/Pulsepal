...
class TestCleanupUtility:
    ...
    def test_dry_run_flag(self, cleanup_script, temp_log_dir):
        ...
        result = subprocess.run(
            [sys.executable, str(cleanup_script), '--dry-run', '--log-dir', str(temp_log_dir)],
            capture_output=True,
            text=True
        )
    ...
    def test_force_flag(self, cleanup_script, temp_log_dir):
        ...
        result = subprocess.run(
            [sys.executable, str(cleanup_script), '--force', '--log-dir', str(temp_log_dir), '--no-archive'],
            capture_output=True,
            text=True
        )
    ...
    def test_archive_flag(self, cleanup_script, temp_log_dir):
        ...
        result = subprocess.run(
            [sys.executable, str(cleanup_script), '--force', '--archive', '--log-dir', str(temp_log_dir)],
            capture_output=True,
            text=True
        )
    ...
    def test_no_archive_flag(self, cleanup_script, temp_log_dir):
        ...
        result = subprocess.run(
            [sys.executable, str(cleanup_script), '--force', '--no-archive', '--log-dir', str(temp_log_dir)],
            capture_output=True,
            text=True
        )
    ...
    def test_older_than_parameter(self, cleanup_script, temp_log_dir):
        ...
        remaining_sessions = [f for f in remaining_sessions if "_searches" not in f.name]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [
                    ...
                    '--archive-dir', str(custom_archive_dir)
                ],
                capture_output=True,
                text=True
            )
