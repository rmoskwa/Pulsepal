...
class TestSemanticRouter:
    ...
    test_cases = [
        ...
        ("Show me an EPI sequence example", QueryRoute.FORCE_RAG, "sequence example request"),
        ("I need a gradient echo implementation", QueryRoute.FORCE_RAG, "implementation request"),
        ...
        ("eco planer imaging example", QueryRoute.FORCE_RAG, "misspelled sequence request"),
        
        # Explicit function mentions - should FORCE_RAG
        ("How does makeTrapezoid work?", QueryRoute.FORCE_RAG, "explicit function"),
        ("What parameters does mr.makeGaussPulse take?", QueryRoute.FORCE_RAG, "function with namespace"),
        ("seq.write('output.seq')", QueryRoute.FORCE_RAG, "code snippet"),
        ("Using seq.addBlock to build sequence", QueryRoute.FORCE_RAG, "function in context"),
        
        # Implementation questions - should FORCE_RAG
        ("How do I implement a gradient echo?", QueryRoute.FORCE_RAG, "implementation question"),
        ...
        ("Debug: my gradient is too small\n```gx = mr.makeTrapezoid()```", QueryRoute.FORCE_RAG, "debug with code"),
        ("Here's my code:\n```matlab\nseq = mr.Sequence()```", QueryRoute.FORCE_RAG, "code block"),
        ...
        ("What causes chemical shift artifacts?", QueryRoute.NO_RAG, "artifact physics"),
        ("Calculate the Ernst angle for TR=100ms", QueryRoute.NO_RAG, "physics calculation"),
        
        # Ambiguous cases - should GEMINI_CHOICE
        ("Why is my image blurry?", QueryRoute.GEMINI_CHOICE, "ambiguous - could be physics or code"),
        ("How can I improve contrast?", QueryRoute.GEMINI_CHOICE, "could be physics or sequence params"),
    ...
    def test_sequence_type_detection(self, mock_router):
        ...
        for seq_type in ['epi', 'gradient echo', 'spin echo', 'mprage']:
            ...
            assert decision.route == QueryRoute.FORCE_RAG
            assert seq_type in decision.search_hints or any(seq_type in hint for hint in decision.search_hints)
    ...
    def test_search_hints_generation(self, mock_router):
        ...
        decision = mock_router.classify_query("Show me an EPI sequence with makeTrapezoid")
        assert len(decision.search_hints) > 0
...
class TestIntegration:
    ...
    def test_complete_classification_flow(self, mock_st):
        ...
        queries = [
            ...
            ("What is T1 relaxation time?", [QueryRoute.NO_RAG, QueryRoute.GEMINI_CHOICE]),
...
@pytest.mark.parametrize("query,expected_route,description", TestSemanticRouter.test_cases)
    ...
    with patch('pulsepal.semantic_router.SentenceTransformer') as mock_st:
        ...
        def mock_encode(texts):
            ...
            for text in texts:
                ...
                if any(kw in text.lower() for kw in ['sequence', 'example', 'implement', 'code']):
                    embedding = np.random.randn(384) + 1  # Shift mean for sequence-related
                elif any(kw in text.lower() for kw in ['t1', 't2', 'relaxation', 'physics']):
        ...
        assert decision.route in [QueryRoute.FORCE_RAG, QueryRoute.NO_RAG, QueryRoute.GEMINI_CHOICE]
        assert decision.confidence > 0
        assert decision.reasoning != ""
        
        # Log the decision for debugging
        logger.info(f"Query: {query[:50]}... -> {decision.route.value} (expected: {expected_route.value})")
