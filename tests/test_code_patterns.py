...
def test_function_clustering():
    ...
    assert len(results['related_functions']) > len(used_functions), "Should find related functions"
...
def test_missing_core_functions():
    ...
    if 'readout_pattern' in clusters:
        ...
        print(f"Missing core functions:")
        for func, reason in missing_core:
            print(f"  - {func}: {reason}")
        
        # Should identify missing ADC
        assert any(func == 'makeAdc' for func, _ in missing_core), "Should identify missing ADC"
...
def test_integration_with_debug_analyzer():
    ...
    print(f"Potentially missing: {clusters.get('potentially_missing', [])[:5]}")  # First 5
