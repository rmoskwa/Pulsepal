...
class TestSizeBasedLimits:
    ...
    def create_test_session_with_size(
        self, log_dir: Path, session_id: str, size_kb: int, days_old: int = 0
    ):
    ...
    def test_identify_sessions_by_size(self, rotation_manager, temp_log_dir):
        ...
        self.create_test_session_with_size(
            temp_log_dir, "session_old_small", 100, days_old=40
        )
        self.create_test_session_with_size(
            temp_log_dir, "session_old_large", 500, days_old=35
        )
        self.create_test_session_with_size(
            temp_log_dir, "session_recent_small", 100, days_old=5
        )
        self.create_test_session_with_size(
            temp_log_dir, "session_recent_large", 500, days_old=10
        )
    ...
    def test_prioritize_oldest_for_deletion(self, rotation_manager, temp_log_dir):
        ...
        for session_id, size_kb, days_old in sessions:
            self.create_test_session_with_size(
                temp_log_dir, session_id, size_kb, days_old
            )
    ...
    async def test_combined_date_and_size_limits(self, rotation_manager, temp_log_dir):
        ...
        self.create_test_session_with_size(
            temp_log_dir, "session_old1", 100, days_old=35
        )
        self.create_test_session_with_size(
            temp_log_dir, "session_old2", 100, days_old=40
        )

        # Create recent large sessions (should be removed by size if needed)
        self.create_test_session_with_size(
            temp_log_dir, "session_recent1", 400, days_old=10
        )
        self.create_test_session_with_size(
            temp_log_dir, "session_recent2", 400, days_old=5
        )

        # Run cleanup
        await rotation_manager.cleanup_old_sessions(dry_run=False)
