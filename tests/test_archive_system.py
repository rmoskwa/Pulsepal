...
class TestArchiveSystem:
    ...
    def create_test_session(
        self, log_dir: Path, session_id: str, content: str = "test", days_old: int = 0
    ):
    ...
    async def test_archive_session_compression(self, rotation_manager, temp_dirs):
        ...
        self.create_test_session(log_dir, session_id, "compress_test")
    ...
    async def test_concurrent_archive_operations(self, rotation_manager, temp_dirs):
        ...
        for session_id in session_ids:
            archive_files = list(
                rotation_manager.archive_dir.rglob(f"{session_id}.tar.gz")
            )
    ...
    async def test_restore_to_custom_directory(self, rotation_manager, temp_dirs):
        ...
        success = await rotation_manager.restore_from_archive(
            session_id, str(restore_dir)
        )
        assert success
    ...
    async def test_archive_compression_ratio(self, rotation_manager, temp_dirs):
        ...
        with open(jsonl_file, "w") as f:
            for i in range(100):
                f.write(
                    json.dumps({"message": "This is a repetitive message " * 10}) + "\n"
                )
        ...
        )  # At least 50% compression for repetitive content
