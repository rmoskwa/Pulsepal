...
class TestDateBasedRotation:
    ...
    def create_test_session(self, log_dir: Path, session_id: str, days_old: int = 0):
        ...
        with open(jsonl_file, "w") as f:
            f.write(
                json.dumps({"message": "test", "timestamp": datetime.now().isoformat()})
                + "\n"
            )
    ...
    def test_parse_timestamp_format(self, rotation_manager):
        ...
        timestamp = rotation_manager._parse_session_timestamp(
    ...
    def test_calculate_file_age(self, rotation_manager, temp_log_dir):
        ...
        old_timestamp = rotation_manager._parse_session_timestamp(
            "session_20240101_120000.jsonl"
        )
        new_timestamp = rotation_manager._parse_session_timestamp(
            "session_20240201_120000.jsonl"
        )
