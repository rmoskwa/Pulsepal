...
class TestLLMSourceSelection:
    ...
    def test_llm_can_specify_multiple_sources(self):
        ...
        test_combinations = [
            ...
            ["api_reference", "crawled_pages", "official_sequence_examples"],  # Comprehensive
        ]
        
        for sources in test_combinations:
            # LLM specifies multiple sources based on query understanding
            assert all(s in ["api_reference", "crawled_pages", "official_sequence_examples"] 
                      for s in sources)
    ...
    def test_default_to_all_sources_when_unspecified(self):
        ...
        # The search_with_source_awareness method should handle this
        # by defaulting to all three sources
        expected_default = ["api_reference", "crawled_pages", "official_sequence_examples"]
...
class TestSourceAwareSearch:
    ...
    async def test_source_aware_search_api(self):
        ...
        assert results["synthesis_hints"][0] == "API documentation provides authoritative function specifications"
    ...
    async def test_source_aware_search_multi(self):
        ...
        # Mock search methods
        rag._search_api_reference = AsyncMock(return_value=[
            {"function_name": "makeBlockPulse", "similarity": 0.8}
        ])
        rag._search_crawled_pages = AsyncMock(return_value=[
...
class TestFormatters:
    ...
    def test_format_api_reference(self):
        ...
        assert "channel (string) [REQUIRED]" in formatted[0]["technical_details"]["parameters"]
    ...
    def test_unified_response_format(self):
        ...
        source_results = {
            "api_reference": [
                {"function_name": "test", "similarity": 0.9}
            ],
            "crawled_pages": [
...
class TestSourceFallback:
    ...
    async def test_fallback_to_secondary(self):
        ...
        rag._search_crawled_pages = AsyncMock(return_value=[
            {"content": "Alternative info", "similarity": 0.6}
        ])
        rag._search_official_sequences = AsyncMock(return_value=[])
        
        # Search should fall back to crawled pages
        results = await rag.search_with_source_awareness(
            query="makeTrapezoid parameters"
        )
...
class TestQueryContext:
    ...
    def test_learning_query_context(self):
        ...
        query = "teach me how to create an EPI sequence"
    ...
    def test_debugging_query_context(self):
        ...
        query = "error when calling makeTrapezoid with wrong parameters"
