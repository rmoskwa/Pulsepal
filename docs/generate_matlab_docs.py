...
class MatlabDocGenerator:
    ...
    def fetch_matlab_functions(self) -> List[Dict[str, Any]]:
        ...
        response = (
            self.supabase.table("api_reference")
            .select(
        ...
        valid_functions = [
            func for func in all_functions if func.get("name") in self.valid_functions
        ]

        print(f"Found {len(all_functions)} total MATLAB functions in database")
        print(
            f"Filtered to {len(valid_functions)} valid functions from function_index.py"
        )

        # Clean up related_functions in each function
        for func in valid_functions:
            if func.get("related_functions"):
                func["related_functions"] = self._clean_related_functions(
                    func["related_functions"]
                )
    ...
    def _fix_examples_namespace(
        self, examples, function_name: str, calling_pattern: str
    ) -> list:
    ...
    def format_parameters(self, parameters: Dict) -> str:
        ...
        if "required" in parameters and parameters["required"]:
            ...
            for param in parameters["required"]:
                ...
                example = (
                    f"`{param.get('example', '')}`" if param.get("example") else ""
                )
                ...
                sections.append(
                    f"| {name} | {param_type} | {description} | {example} | {units} |"
                )
        ...
        if "optional" in parameters and parameters["optional"]:
            ...
            for param in parameters["optional"]:
                ...
                sections.append(
                    f"| {name} | {param_type} | {default} | {description} | {example} |"
                )
    ...
    def generate_function_doc(
        self, func: Dict[str, Any], existing_functions: set
    ) -> str:
        ...
        if usage_examples and isinstance(usage_examples, list):
            usage_examples = self._fix_examples_namespace(
                usage_examples, name, calling_pattern
            )
        ...
        if returns:
            ...
            if isinstance(returns, list):
                ...
                if "name" in returns:
                    ...
                    content.append("|--------|------|-------------|")
                    content.append(
                        f"| `{returns.get('name', 'output')}` | {returns.get('type', '')} | {returns.get('description', '')} |"
                    )
                    content.append("")
                else:
        ...
        if related_functions:
            ...
            if isinstance(related_functions, list):
                ...
                related_text = self.fix_function_links(
                    str(related_functions), existing_functions
                )
                            ...
                            "Functions": [
                                {
                                    func.get(
                                        "name", "Unknown"
                                    ): f"matlab_api/{func.get('name', 'unknown')}.md"
                                }
