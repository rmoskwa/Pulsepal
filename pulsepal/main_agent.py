...
def _register_tools():
    ...
    def log_tool_usage(tool_func):
        """Decorator to log tool usage and responses."""
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Log input parameters (truncate if too long)
            try:
                # Extract meaningful parameters to log
                if args and len(args) > 1:
                    # Skip context, log other args
                    log_args = args[1:] if len(args) > 1 else []
                    if log_args:
                        arg_str = str(log_args)[:500]
                        logger.info(f"  📥 Input: {arg_str}...")
                if kwargs:
                    kwarg_str = str(kwargs)[:500]
                    logger.info(f"  📥 Kwargs: {kwarg_str}...")
            except Exception as e:
                logger.debug(f"  Could not log tool inputs: {e}")
            ...
            try:
                ...
                logger.debug(f"  📤 Response preview: {result_str}...")
                ...
                logger.error(
                    f"  ❌ Tool {tool_name} failed with {error_type}: {str(e)[:500]}"
                )
                raise
    ...
    pulsepal_agent.tool(wrapped_execute_query)
