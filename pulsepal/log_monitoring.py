        ...
        try:
            ...
            if len(old_sessions) > 50:
                health["issues"].append(
                    f"Many old sessions pending cleanup: {len(old_sessions)}"
                )
                if health["status"] == "healthy":
                    health["status"] = "warning"
        except Exception as e:
    ...
    def get_metrics(self) -> Dict:
        ...
        try:
            ...
            for session_file in all_sessions:
                timestamp = self.rotation_manager._parse_session_timestamp(
                    session_file.name
                )
                if timestamp:
            ...
            metrics["storage"]["usage_percent"] = (
                (current_size / self.rotation_manager.max_size_bytes * 100)
                if self.rotation_manager.max_size_bytes > 0
                else 0
            )
            ...
            if self.rotation_manager.archive_dir.exists():
                archive_files = list(
                    self.rotation_manager.archive_dir.rglob("*.tar.gz")
                )
                metrics["storage"]["archived_sessions"] = len(archive_files)
                metrics["storage"]["archive_size_bytes"] = sum(
                    f.stat().st_size for f in archive_files
                )

            # Rotation settings
            metrics["rotation"]["retention_days"] = self.rotation_manager.retention_days
            metrics["rotation"]["importance_threshold"] = (
                self.rotation_manager.importance_threshold
            )
