...
class OptimizedRAGMethods:
    ...
    async def get_official_sequence_optimized(
        ...
        try:
            ...
            summary_result = (
                self.client.from_("official_sequence_examples")
    ...
    async def search_crawled_pages_optimized(
        ...
        try:
            ...
            if not search_result.data:
                ...
                search_result = (
                    self.client.from_("crawled_pages")
                    .select("id, url, title")
                    .ilike("title", f"%{query}%")
                    .limit(limit)
                    .execute()
...
def apply_optimizations_to_rag_service(rag_service):
    ...
    rag_service.search_crawled_pages_optimized = (
        optimizer.search_crawled_pages_optimized
    )
