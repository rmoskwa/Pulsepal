...
def load_api_keys() -> Dict[str, dict]:
    ...
    if api_keys_json:
        logger.info(f"Found ALPHA_API_KEYS environment variable (length: {len(api_keys_json)})")
        try:
        ...
        logger.info("No ALPHA_API_KEYS environment variable found, using default test key")
