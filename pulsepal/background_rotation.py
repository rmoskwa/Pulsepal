...
class BackgroundRotationService:
    ...
    async def stop(self):
        ...
        if self._task:
            try:
                ...
                logger.warning("Background rotation task did not stop in time, cancelling")
                self._task.cancel()
                try:
                    await self._task
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(
                        ...
                        timeout=self.rotation_interval
                    )
                except asyncio.TimeoutError:
    ...
    async def _perform_rotation(self):
        ...
        try:
            ...
            if usage_percent >= 90:
                logger.warning(f"Log directory at {usage_percent:.1f}% of capacity ({current_size / 1024**2:.1f}MB / {max_size / 1024**2:.1f}MB)")
            ...
            if old_sessions or current_size > max_size:
                logger.info(f"Starting automatic cleanup: {len(old_sessions)} old sessions, size: {current_size / 1024**2:.1f}MB")
                
                stats = await self.rotation_manager.rotate_logs()
                
                if stats.get('errors'):
                    logger.error(f"Rotation completed with {len(stats['errors'])} errors")
                else:
                    logger.info(
