...
def build_validation_error_message(
    ...
    # Report functions that don't exist
    if not_found:
        ...
        for func_call, validation in not_found:
            ...
            # Only show known corrections from COMMON_HALLUCINATIONS
    ...
    message_parts.append("**Available tools to find correct functions:**\n")

    message_parts.append("1. **Search for functions by purpose:**")
    message_parts.append(
        "   `search_pulseq_knowledge(query='[describe what you want to do]', table='api_reference', limit=3)`"
    )
    message_parts.append("   This will return the top 3 functions with:")
    message_parts.append("   - Full function signatures and descriptions")
    message_parts.append("   - Required and optional parameters with types")
    message_parts.append("   - Return values and usage examples\n")

    message_parts.append("2. **Find usage patterns in real code:**")
    message_parts.append(
        "   `search_pulseq_knowledge(query='[concept or operation]', table='sequence_chunks', limit=5)`"
    )
    message_parts.append(
        "   Returns actual code segments showing how functions are used together\n"
    )
