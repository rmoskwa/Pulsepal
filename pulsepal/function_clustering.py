...
class FunctionClustering:
    ...
    def get_related_functions(
        self, valid_functions: List[str], corrected_functions: List[str]
    ) -> Dict:
        ...
        if "readout_pattern" in clusters_used:
            context["usage_hints"].append(
                "Consider adding gradient prephasing before readout"
            )
        if "rf_excitation" in clusters_used:
            context["usage_hints"].append(
                "Remember to include slice selection gradient with RF"
            )
        if "epi_readout" in clusters_used:
            context["usage_hints"].append(
                "EPI requires gradient blips between readouts"
            )
        if "diffusion_weighting" in clusters_used:
            context["usage_hints"].append(
                "Ensure b-value calculation matches gradient timing"
            )
