...
class LogRotationManager:
        ...
        async with self._lock:
            try:
                ...
                await loop.run_in_executor(
                    None, self._compress_files, files_to_archive, archive_path
                )
    ...
    async def cleanup_old_sessions(
        self, dry_run: bool = False, archive_important: bool = True
    ) -> Dict[str, any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Found {len(old_sessions)} sessions older than {self.retention_days} days"
            )
            ...
            for session_id in old_sessions:
                try:
                    ...
                    if not dry_run:
                        ...
                        if (
                            archive_important
                            and importance_score >= self.importance_threshold
                        ):
                        ...
                        logger.info(
                            f"[DRY RUN] Would {action} {session_id} (score: {importance_score:.2f}, size: {session_size} bytes)"
                        )
                        stats["sessions_removed"] += 1
                        stats["space_freed_bytes"] += session_size
                        if importance_score >= self.importance_threshold:
            ...
            if current_size > self.max_size_bytes:
                logger.warning(
                    f"Size limit exceeded: {current_size / 1024**3:.2f}GB > {self.max_size_bytes / 1024**3:.2f}GB"
                )
                    ...
                    if session_id not in old_sessions:  # Don't double-count
                        try:
                            ...
                            if not dry_run:
                                ...
                                logger.info(
                                    f"[DRY RUN] Would {action} {session_id} for size limit (score: {importance_score:.2f}, size: {size} bytes)"
                                )
                                stats["sessions_removed"] += 1
                                stats["space_freed_bytes"] += size
                                if importance_score >= self.importance_threshold:
            ...
            # Log summary
            logger.info(
                f"Cleanup complete: Archived {stats['sessions_archived']}, Removed {stats['sessions_removed']}, Freed {stats['space_freed_bytes'] / 1024**2:.2f}MB"
            )
    ...
    async def restore_from_archive(
        self, session_id: str, target_dir: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
                None, self._extract_archive, archive_file, target
            )
