    ...
    for result in results:
        formatted_entry = {
            ...
            "function": {
                ...
                "usage": result.get('correct_usage', result.get('calling_pattern', '')),  # HOW to call it correctly
            },
...
def format_single_chunk(chunk: Dict) -> Dict:
    ...
    return {
        ...
        "content": {
            ...
            "url": url,
            "title": extract_title_from_url(url) if url else chunk.get('title', 'Untitled'),
        },
...
def parse_ai_summary(summary: str) -> Dict[str, Any]:
    ...
    # Extract warnings
    if 'warning' in summary_lower or 'caution' in summary_lower or 'note' in summary_lower:
...
def format_unified_response(source_results: Dict[str, List], query_context: Dict) -> Dict:
    """
...
def generate_synthesis_recommendations(source_results: Dict, query_context: Dict) -> List[str]:
    """
    ...
    is_learning = any(word in query_lower for word in ['learn', 'tutorial', 'how to', 'teach', 'example'])
    is_parameter = any(word in query_lower for word in ['parameter', 'argument', 'signature', 'input', 'output'])
    is_debug = any(word in query_lower for word in ['error', 'wrong', 'fix', 'debug', 'issue'])
    ...
    if has_api and has_examples:
        recommendations.append("Combine API specs with practical examples for complete understanding")
    
    if has_tutorials:
        recommendations.append("Official examples provide best practices and tested implementations")
